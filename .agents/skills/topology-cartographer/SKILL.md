...
- delegate per-directory extraction to the `topology-extractor` custom subagent;
...
Delegate to the `topology-extractor` custom subagent **only** when a single scan is
...
a custom subagent adds nothing: it would re-read the same file to reach the conclusion
