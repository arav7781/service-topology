...
Invoke as `/topology-cartographer <mode> [args]` where the installed Claude Code
version supports user-invocable skills, or in natural language ("use the
...
Delegate to the `topology-extractor` subagent **only** when a single scan is
...
a subagent adds nothing: it would re-read the same file to reach the conclusion
...
**Subagents** (project `agents/` directory): `topology-extractor`,
